...
function chatHeartbeat() {
	global $session,$database,$active_users;
...
?>
{
		"items": [
			...
			<?php echo $users;?>
                ...
                "usercount": "<? echo count($active_users); ?>"
}
...
EOD;
	...
	$sql = "insert into chat (chat.from,chat.to,message,sent) values ('".mysql_real_escape_string($from)."', '".mysql_real_escape_string($to)."','".mysql_real_escape_string($message)."', '".date("Y-m-d H:i:s")."')";
	$query = $database->query($sql);
	echo "1";
	exit(0);
}
