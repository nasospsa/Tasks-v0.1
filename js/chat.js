...
function restructureChatBoxes() {
	...
	for (x in chatBoxes) {
		...
		if ($("#chatbox_"+chatboxtitle).css('display') != 'none') {
			if (align == 0) {
				$("#chatbox_"+chatboxtitle).css('right', '260px');
			} else {
				width = (align)*(225+7)+260;
...
function createChatBox(chatboxtitle,minimizeChatBox) {
	...
	if (chatBoxeslength == 0) {
		...
		width = (chatBoxeslength)*(225+7)+260;
		$("#chatbox_"+chatboxtitle).css('right', width+'px');
	}
	  ...
	  success: function(data) {
		...
		if (itemsfound > 0) {
			...
			if (chatHeartbeatTime > maxChatHeartbeat) {
                ...
                //update Userlist
                var list_item;
                $("#chat_list").html("<ul></ul>");
                $.each(data.userlist, function(i,item){
                    if (item){ // fix strange ie bug
                        list_item = "<li><a href=\"javascript:void(0)\" onClick=\"javascript:chatWith('"+item.username+"')\">"+item.username+"</a></li>";
                        $("#chat_list ul").append(list_item);
                    }
                });
                $("#chat_list_header_title").html("Chat ("+data.usercount+")");
                if (data.usercount>0){
                    $("#chat_active").attr("class","chat_on");
                }
                else{
                    $("#chat_active").attr("class","");
                }
                var users_logged = parseInt(data.usercount)+1;
                $("#current_users").html(users_logged);
